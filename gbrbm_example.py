...
gbrbm = GBRBM(VISIBLE_UNITS, HIDDEN_UNITS, CD_K, use_cuda=CUDA)
...
for epoch in range(EPOCHS):
    ...
    for batch, _ in train_loader:
        ...
        batch_error = gbrbm.contrastive_divergence(batch)
...
for i, (batch, labels) in enumerate(train_loader):
    ...
    train_features[i*BATCH_SIZE:i*BATCH_SIZE+len(batch)] = gbrbm.sample_hidden(batch).cpu().numpy()
...
for i, (batch, labels) in enumerate(test_loader):
    ...
    test_features[i*BATCH_SIZE:i*BATCH_SIZE+len(batch)] = gbrbm.sample_hidden(batch).cpu().numpy()
